...
from fast.cg_node import CalleeNode, CGNode
from fast.ddg_constructor import merge_def_use
...
def list_all_callees(cpg: DiGraph, node: str, dataset_name: str) -> dict:
    ...
    while not queue.is_empty():
        ...
        callees = extract_callees(cpg, current_node, dataset_name)
...
def extract_callees(cpg: DiGraph, node: str, dataset_name: str) -> list:
    ...
    while not queue.is_empty():
        ...
        if cpg.nodes[current_node]['cpg_node'].node_type == 'method_invocation':
            if not list(cpg.successors(current_node)):
                continue
            callee_node = extract_single_callee(cpg, node, current_node, dataset_name)
...
def extract_single_callee(cpg: DiGraph, stat_node: str, invocation_node: str, dataset_name: str):
    ...
    def find_ast_parent(cpg: DiGraph, node: str):
        ...
        while not queue.is_empty():
            ...
            if len(current_node_parents) != 1:
                logger.error(f'Node {current_node} has {len(current_node_parents)} parents in cg constructor.')
                exit(-1)
    ...
    for _args in _callee_args:
        ...
        if _args_type in int_dict:
            ...
            continue
    block_key = stat_node
    if not cpg.nodes[block_key]['cpg_node'].match_block:
        block_key = find_ast_parent(cpg, stat_node)
    if block_key is None:
        logger.error(f'Cannot find block id for callee node')
        exit(-1)

    callee_node = CalleeNode(stat_node, block_key, call_name, callee_args_num, callee_args_type, dataset_name)
