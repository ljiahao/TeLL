...
def visualize_ast_cfg(ast_cpg: nxDiGraph = None, dot_name: str = None, ast_display: bool = True) -> bool:
    """Visualize ast cpg.
    ...
    for node in nodes:
        ...
        if ast_display:
            if info.match_statement and info.match_block:
                color = 'Red'
            elif info.match_statement:
                color = 'Green'
            else:
                color = 'Black'
    ...
    dot.render(filename=dot_file, view=False)
    command = 'rsvg-convert -f pdf -o ' + dot_name + '.pdf ' + dot_file + '.svg; rm ' + dot_file + '.svg; rm ' + dot_file
    os.system(command)
    return True